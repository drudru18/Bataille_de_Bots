    ...
    def write_log(self, name, total_time, win1, lose1, win2, lose2):
        ...
        if name in content:
            ...
            if content[name]["Bot 1 Lose"] == 0:
                content[name]["Ratio Bot 1"] = 1
            else:
                content[name]["Ratio Bot 1"] = round(content[name]["Bot 1 Win"]/(content[name]["Bot 1 Win"]+content[name]["Bot 1 Lose"]), 2)
            if content[name]["Bot 2 Lose"] == 0:
                content[name]["Ratio Bot 2"] = 1
            else:
                content[name]["Ratio Bot 2"] = round(content[name]["Bot 2 Win"]/(content[name]["Bot 2 Win"]+content[name]["Bot 2 Lose"]), 2)
        else:
            data = {
